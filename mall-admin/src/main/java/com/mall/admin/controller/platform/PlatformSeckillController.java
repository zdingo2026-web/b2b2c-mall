...
@Slf4j
@RestController
@RequestMapping("/api/v1/platform/seckill")
...
public class PlatformSeckillController {
    ...
    @GetMapping("/list")
    public R<PageResult<SeckillActivityVO>> list(
            @RequestParam(required = false) Integer status,
            @RequestParam(defaultValue = "1") Integer page,
            @RequestParam(defaultValue = "10") Integer limit) {
        return R.ok(seckillActivityService.list(null, status, page, limit));
    }
    ...
    public R<Void> create(@Validated @RequestBody SeckillActivityCreateDTO dto) {
        seckillActivityService.create(0L, dto);
        return R.ok();
    }
