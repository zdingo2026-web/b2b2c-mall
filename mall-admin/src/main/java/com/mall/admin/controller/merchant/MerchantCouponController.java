...
public class MerchantCouponController {
    ...
    @GetMapping("/list")
    public R<PageResult<CouponTemplateVO>> list(@RequestParam(required = false) Integer couponType,
                                                 @RequestParam(required = false) Integer status,
                                                 @RequestParam(defaultValue = "1") Integer page,
                                                 @RequestParam(defaultValue = "10") Integer limit) {
        Long tenantId = UserContext.getTenantId();
        return R.ok(couponTemplateService.list(tenantId, couponType, status, page, limit));
    }
    ...
    public R<Void> create(@Validated @RequestBody CouponTemplateCreateDTO dto) {
        Long tenantId = UserContext.getTenantId();
        couponTemplateService.create(tenantId, dto);
        return R.ok();
    }
