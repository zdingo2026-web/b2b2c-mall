<template>
  <div>
    <div class="flex items-center justify-between mb-4">
      <h2 class="text-lg font-bold">{{ isEdit ? '编辑商品' : '新增商品' }}</h2>
...
async function loadDetail(id: string) {
  ...
  try {
    const detail: SpuDetailVO = await getPlatformSpuDetail(id)
    form.value = {
...
async function handleSubmit() {
  ...
  try {
    if (isEdit && route.query.id) {
      await updatePlatformSpu(route.query.id as string, form.value as any)
      ElMessage.success('修改成功')
    } else {
      await createPlatformSpu(form.value as any)
...
onMounted(async () => {
  const [tree, brandsData] = await Promise.all([getCategoryTree(), getBrandList()])
