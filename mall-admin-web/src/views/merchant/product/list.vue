<template>
  <div>
    <h2 class="text-lg font-bold mb-4">商品列表</h2>
    <el-card>
      ...
      <el-table :data="tableData" stripe v-loading="loading">
          ...
          <template #default="{ row }">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-xs text-gray-400">{{ row.categoryName }}</p>
              </div>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="row.status === 0 || row.status === 3" link type="success" size="small" @click="handlePublish(row)">上架</el-button>
...
async function fetchList() {
  loading.value = true
  try {
    const data = await getMerchantSpuList(queryParams.value)
...
function handleEdit(row: SpuVO) {
  router.push(`/merchant/product/add?id=${row.id}`)
}

async function handlePublish(row: SpuVO) {
  await publishMerchantSpu(row.id)
...
async function handleUnpublish(row: SpuVO) {
  ...
  await unpublishMerchantSpu(row.id)
  ElMessage.success('已下架')
  fetchList()
}

async function handleDelete(row: SpuVO) {
  await ElMessageBox.confirm(`确定删除商品「${row.productName}」？删除后不可恢复！`, '警告', { type: 'warning' })
  await deleteMerchantSpu(row.id)
  ElMessage.success('已删除')
  fetchList()
}

onMounted(async () => {
  categoryOptions.value = await getMerchantCategoryTree()
  fetchList()
})
</script>
