  ...
  <div>
    ...
    <el-card>
      <div class="mb-4 flex gap-4 flex-wrap">
        ...
        <el-button type="success" @click="showCreateDialog">新增秒杀活动</el-button>
      </div>

      <el-table :data="tableData" stripe v-loading="loading">
        <el-table-column prop="id" label="ID" width="80" />
        <el-table-column prop="activityName" label="活动名称" min-width="160" />
        <el-table-column label="秒杀时间" min-width="180">
...
import {
  ...
  deleteMerchantSeckill,
  startMerchantSeckill,
  endMerchantSeckill,
...
async function fetchList() {
  loading.value = true
  try {
    const data = await getMerchantSeckillList(queryParams.value)
...
function showCreateDialog() {
  isEdit.value = false
  editingId.value = 0
  dialogForm.value = { activityName: '', dateRange: null, limitPerUser: 1, description: '' }
...
async function showEditDialog(row: any) {
  ...
  try {
    const detail = await getMerchantSeckillDetail(row.id)
    dialogForm.value = {
...
async function handleSubmit() {
  ...
  try {
    ...
    if (isEdit.value) {
      await updateMerchantSeckill(editingId.value, payload)
...
async function handleStart(row: any) {
  ...
  await startMerchantSeckill(row.id)
  ElMessage.success('活动已开始')
  fetchList()
}

async function handleEnd(row: any) {
  await ElMessageBox.confirm(`确定结束秒杀活动「${row.activityName}」？结束后不可恢复！`, '提示', { type: 'warning' })
  await endMerchantSeckill(row.id)
  ElMessage.success('活动已结束')
  fetchList()
}

async function handleDelete(row: any) {
  await ElMessageBox.confirm(`确定删除秒杀活动「${row.activityName}」？删除后不可恢复！`, '警告', { type: 'warning' })
  await deleteMerchantSeckill(row.id)
